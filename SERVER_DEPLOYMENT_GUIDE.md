...
If you are having issues with your certificate:

1. Run the diagnostic script:
   ```bash
   bash diagnose_ssl.sh
   ```

2. If the certificate is missing or expired, run Certbot on the host:
   ```bash
   certbot certonly --standalone -d yourdomain.com
   ```

3. Update `frontend/nginx-ssl.conf` with your domain and restart:
   ```bash
   docker compose -f docker-compose.prod.yml up -d --build
   ```
