...
export function ManageStaffMembers() {
    ...
    const handleGenerateLink = async () => {
        if (!formData.branchId) {
            toast.error(t('messages.selectBranchFirst'));
            return;
        }
        ...
        try {
            ...
            toast.success(t('messages.invitationLinkCreated'));
        } catch (error) {
            toast.error(t('messages.linkCreationError'));
        } finally {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            toast.success(t('messages.staffUpdated'));
            setIsAdding(false);
            setEditingId(null);
    ...
    const handleDelete = async (id: string) => {
        if (window.confirm(t('messages.confirmDeleteStaff'))) {
            try {
                await deleteStaffMember(id);
                toast.success(t('messages.staffDeleted'));
            } catch (error) {
                toast.error(t('common.error'));
            }
        }
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-6">
            ...
            <div className="bg-blue-700 dark:bg-blue-800 px-4 pb-6 pt-6 text-white text-center">
                <div className="mb-4 flex items-center justify-between">
                    ...
                    <h1 className="text-xl font-semibold">{t('profile.staffList')}</h1>
                        ...
                        <div className="space-y-3">
                            <div>
                                <label className="text-xs text-gray-500 mb-1 block">
                                    {editingId ? t('common.name') : t('admin.username') + " (Lotincha, bo'sh joylarsiz)"}
                                </label>
                                <Input
                                    placeholder={editingId ? t('admin.staffName') : t('admin.usernameExample')}
                                    value={formData.name}
                                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                    className="dark:bg-gray-700"
                                />
                                {!editingId && <p className="text-[10px] text-gray-400 mt-1">{t('admin.usernameNote')}</p>}
                            </div>

                            <div>
                                <label className="text-xs text-gray-500 mb-1 block">{t('common.branch')}</label>
                                <select
                                    className="w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                    value={formData.branchId}
                                    onChange={(e) => setFormData({ ...formData, branchId: e.target.value })}
                                >
                                    <option value="">{t('seller.selectBranch')}</option>
                                    {branches.map(b => (
                            ...
                            {!editingId && !invitationLink && (
                                ...
                                >
                                    {isGenerating ? t('common.generating') : t('admin.generateLink')}
                                </Button>
                            )}
                            ...
                            {editingId && (
                                <div className="flex gap-2 pt-2">
                                    <Button variant="outline" className="flex-1" onClick={() => setIsAdding(false)}>{t('common.cancel')}</Button>
                                    <Button className="flex-1 bg-blue-600 hover:bg-blue-700 text-white" onClick={handleSave}>{t('common.save')}</Button>
                                </div>
                            )}

                            {/* Close button for Add mode */}
                            {!editingId && (
                                <Button variant="outline" className="w-full mt-2" onClick={() => { setIsAdding(false); setInvitationLink(null); }}>{t('common.close')}</Button>
                            )}

                            {invitationLink && (
                                <div className="mt-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                                    <p className="text-xs text-green-700 dark:text-green-400 mb-2 font-medium">{t('admin.linkReady')}</p>
                                    ...
                                    <p className="text-[10px] text-gray-500 mt-2 italic">{t('admin.linkNote')}</p>
                                </div>
                            )}
                        </div>
                    </Card>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center text-xs text-gray-500 dark:text-gray-400">
                                                ...
                                                {staff.isActive ? (
                                                    <span className="text-green-600 flex items-center"><CheckCircle className="h-3 w-3 mr-0.5" /> {t('common.active')}</span>
                                                ) : (
                                                    <span className="text-gray-400 flex items-center"><XCircle className="h-3 w-3 mr-0.5" /> {t('common.inactive')}</span>
                                                )}
                                            </div>
                                        </div>
                                    </div>
